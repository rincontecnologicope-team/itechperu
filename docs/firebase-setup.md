...
## 1) Habilitar Firestore (Storage opcional)

En Firebase Console:

1. Abre el proyecto `itechperu-1d1d4`.
2. Crea Firestore Database (modo produccion).
3. Opcional: habilita Firebase Storage solo si usaras `IMAGE_STORAGE_PROVIDER=firebase`.
...
Si usaras Cloudinary para imagenes:
...
Si prefieres Firebase Storage:

```bash
IMAGE_STORAGE_PROVIDER=firebase
FIREBASE_STORAGE_BUCKET=itechperu-1d1d4.appspot.com
```
